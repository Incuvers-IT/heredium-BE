...
public class CloudMail {
    ...
    private String makeSignature(HttpMethod method, String epochTimestamps, String url) throws Exception {
        String space = " ";  // 공백
        String newLine = "\n";  // 줄바꿈
...
//            log.info(resultMap.toString());
    ...
    private String smsRequest(List<ReserveModel> list, LocalDateTime reserveTime) {
        try {
            ...
            List<Message> receivers = list.stream()
                    .map(m -> new Message(m.getPhone().replace("-", "").trim(), m.getSubject(), m.getContents()))
            ...
            factory.setReadTimeout(5000);
            RestTemplate restTemplate = new RestTemplate(factory);
            ResponseEntity<NCloudSMSResponse> result = restTemplate.postForEntity(uri.toString(), entity, NCloudSMSResponse.class);
    ...
    private String smsReserveCancel(String smsRequestId) {
        try {
            String url = API_SMS_URL;
            String apiUrl = String.format("/sms/v2/services/%s/reservations/%s", SMS_SERVICE_ID, smsRequestId);
            String epochTimestamps = String.valueOf(Instant.now().toEpochMilli());
            HttpHeaders header = getNCloudHeader(apiUrl, epochTimestamps);
