...
public class NCloudSMSBuilder extends NCloudSMSRequestModel {
  ...
  /**
   * MMS 필수 해상도는 최대 1500 * 1440, 공백 사용 불가
   *
  ...
  public NCloudSMSRequest build() {
    if (!StringUtils.isNullOrEmpty(this.reserveTime)
        || !StringUtils.isNullOrEmpty(this.scheduleCode)) {
      if (StringUtils.isNullOrEmpty(this.content)) {
        throw new NCloudKitException(
            NCloudKitErrorCode.SMS_CONTENT_INVALID, "예약 발송시에는 Root의 Content가 존재하여야 합니다.");
      }
    }
