...
public class SignatureHeader {
    ...
    public String toSignature(Credentials credentials) {
        ...
        StringBuilder message = new StringBuilder()
                .append(this.method).append(" ").append(pathWithQuery).append("\n")
                .append(this.epochTimestamps).append("\n")
                .append(credentials.getAccessKey());

        try {
            SecretKeySpec signingKey = new SecretKeySpec(credentials.getSecretKey().getBytes(StandardCharsets.UTF_8), "HmacSHA256");
...
//            return Base64.encodeBase64String(rawHmac); // InipaySample_v.1.3.jar 에서 덮어씌워 해당 코드가 작동안됨
        } catch (Exception e) {
            throw new NCloudKitException(NCloudKitErrorCode.SDK_ERROR, "Failed to make signature for IAM credentials: " + e.getMessage());
        }
    }
