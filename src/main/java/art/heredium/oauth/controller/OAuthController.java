...
public class OAuthController {
    ...
    private void initBinder(final WebDataBinder webdataBinder) {
        webdataBinder.registerCustomEditor(OAuth2Provider.class, new PropertyEditorSupport() {
            @Override
    ...
    public RedirectView authorization(RedirectView redirectView, @PathVariable(value = "provider") OAuth2Provider provider) {
        redirectView.setUrl(provider.getLoginUrl(oAuth2Properties.getRegistration()));
        return redirectView;
    }

    @PostMapping("/{provider}")
    public ResponseEntity login(HttpServletResponse response, @PathVariable(value = "provider") OAuth2Provider provider, @RequestParam(value = "code") String code) {
