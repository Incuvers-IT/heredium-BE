...
public class OAuthService {
    ...
    public String getToken(OAuth2Provider provider, String code) {
        ClientRegistration clientRegistration = oAuth2Properties.getRegistration().get(provider.name().toLowerCase());
        ...
        try {
            ResponseEntity<Map> responseEntity = restTemplate.postForEntity(clientRegistration.getTokenUri(), parameters, Map.class);
            if (responseEntity.getStatusCode() != HttpStatus.OK || responseEntity.getBody() == null) {
                throw new ApiException(ErrorCode.BAD_VALID);
            }
            return provider == OAuth2Provider.APPLE ? (String) responseEntity.getBody().get("id_token") : (String) responseEntity.getBody().get("access_token");
        } catch (Exception e) {
            throw new ApiException(ErrorCode.BAD_VALID);
        }
    }

    public PostLoginResponse loginByCode(HttpServletResponse response, OAuth2Provider provider, String code) {
        String token = getToken(provider, code);
        return loginByToken(response, provider, token);
    }

    public PostLoginResponse loginByToken(HttpServletResponse response, OAuth2Provider provider, String token) {
        ...
        AuthToken accessToken = authTokenProvider.createAuthToken(userId, userPrincipal.getAuth(), new Date(now.getTime() + appProperties.getAuth().getAccessTokenExpiry().toMillis()));
        AuthToken refreshToken = authTokenProvider.createAuthToken(userId, userPrincipal.getAuth(), new Date(now.getTime() + appProperties.getAuth().getRefreshTokenExpiry().toMillis()));
        jwtRedisUtil.setDataExpire(refreshToken.getToken(), userId, appProperties.getAuth().getRefreshTokenExpiry().getSeconds());
        ...
        return new PostLoginResponse(accessToken.getToken(), userPrincipal.getIsSleeper(), userPrincipal.getIsSleeper() ? userPrincipal.getName() : null);
    }

    public PostLoginResponse insert(HttpServletRequest request, HttpServletResponse response, OAuth2Provider provider, PostAccountSnsRequest dto) {
        ...
        if (provider == OAuth2Provider.KAKAO) {
            ...
            try {
                ...
                isAllow = kaKaoUserTerms.getAllowed_service_terms().stream().anyMatch(x -> x.getTag().equals("marketing"));
            } catch (Exception e) {
                isAllow = false;
            }
    ...
    private PostLoginResponse insertAndLogin(HttpServletRequest request, HttpServletResponse response, OAuth2Provider provider, String token, Account entity) {
    ...
    private OAuth2UserInfo getUserInfo(OAuth2Provider provider, String token) {
        ...
        if (provider == OAuth2Provider.APPLE) {
            Map<String, Object> userId = provider.getUserInfoFromJwt(oAuth2Properties.getRegistration(), token);
            return provider.getUserInfo(userId);
        } else {
            String userInfoUri = provider.getClientRegistration(oAuth2Properties.getRegistration().get(provider.name().toLowerCase())).getUserInfoUri();
            Map map = requestRestTemplate(HttpMethod.POST, userInfoUri, token, null, Map.class);
            return provider.getUserInfo(map);
        }
    }

    private <T> T requestRestTemplate(HttpMethod method, String url, String token, MultiValueMap<String, String> map, Class<T> cls) {
        RestTemplate restTemplate = new RestTemplate();
        ...
        try {
            ResponseEntity<T> responseEntity = map == null
    ...
    public KakaoUserTermsResponse getKaKaoUserTerms(OAuth2Provider provider, String token) {
        ...
        String userTermsUri = provider.getClientRegistration(oAuth2Properties.getRegistration().get(provider.name().toLowerCase())).getUserTermsUri();
        return requestRestTemplate(HttpMethod.GET, userTermsUri, token, null, KakaoUserTermsResponse.class);
    }
}
