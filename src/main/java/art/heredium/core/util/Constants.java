...
public class Constants {
    ...
    public static Gson getGson() {
        Gson gson = new GsonBuilder().serializeNulls().registerTypeAdapter(LocalDateTime.class, new GsonLocalDateTimeAdapter()).create();
        return gson;
    }
    ...
    public static List<String> getEditorTempFile(String contents, String path) {
        List<String> list = getImageNameFromHtml(contents).stream()
    ...
    public static void moveFileFromTemp(CloudStorage cloudStorage, Storage storage, String fileFolderPath) {
        if (storage != null) {
            ...
            if (resizeImage != null) {
                if (resizeImage.getSmall() != null) {
                    String movedSmallFile = moveTempFile(cloudStorage, fileFolderPath, resizeImage.getSmall());
                    if (movedSmallFile != null) {
                        resizeImage.setSmall(movedSmallFile);
                    }
                }
                if (resizeImage.getMedium() != null) {
                    String movedMediumFile = moveTempFile(cloudStorage, fileFolderPath, resizeImage.getMedium());
                    if (movedMediumFile != null) {
                        resizeImage.setMedium(movedMediumFile);
                    }
                }
                if (resizeImage.getLarge() != null) {
                    String movedLargeFile = moveTempFile(cloudStorage, fileFolderPath, resizeImage.getLarge());
    ...
    private static String moveTempFile(CloudStorage cloudStorage, String fileFolderPath, String savedFileName) {
    ...
    public static boolean valid(MultipartFile multipartFile, FilePathType type) {
        ...
        if (type.getMegabytes() != null && multipartFile.getSize() > type.getMegabytes() * 1024 * 1024) {
            throw new ApiException(ErrorCode.MAX_UPLOAD_SIZE, "파일 용랑초과");
        }

        String extension = FilenameUtils.getExtension(multipartFile.getOriginalFilename());
        if (extension == null || type.getExtension() != null && type.getExtension().stream().noneMatch(extension::equalsIgnoreCase)) {
    ...
    public static String emailMasking(String email) {
        ...
        return buf.replace(start, end, String.join("", Collections.nCopies(end - start, "*"))) + (split.length > 1 ? ("@" + split[1]) : "");
    }

    public static String getUUID() {
        return getNow().format(DateTimeFormatter.ofPattern("yyMMdd")) + RandomStringUtils.randomAlphanumeric(10);
    }
