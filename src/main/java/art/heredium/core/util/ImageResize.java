...
public final class ImageResize {
    ...
    private static MultipartImage resizeImage(MultipartFile file, BufferedImage originalImage, int width, int height) throws IOException {
        Image resizeImage = originalImage.getScaledInstance(width, height, Image.SCALE_SMOOTH);
        int imgType = (originalImage.getTransparency() == Transparency.OPAQUE) ? BufferedImage.TYPE_INT_RGB : BufferedImage.TYPE_INT_ARGB;
