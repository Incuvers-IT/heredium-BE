        ...
        try {
            ...
            if ("0000".equals(dto.getResultCode())) {
                ...
                try {
                    ...
                    InicisPayResponse response = new Gson().fromJson(authResultString, InicisPayResponse.class);

                    //String test = authResultString.replace(",", "&").replace(":", "=").replace("\"", "").replace(" ", "").replace("\n", "").replace("}", "").replace("{", "");

//                    Map<String, String> resultMap = ParseUtil.parseStringToMap(test); //문자열을 MAP형식으로 파싱
                    ...
                    if ("0000".equals(response.getResultCode()) && secureSignature.equals(response.getAuthSignature())) {    //결제보안 강화 2016-05-18
                        /*****************************************************************************
                        ...
                        //결제보안키가 다른 경우
                        if (!secureSignature.equals(response.getAuthSignature()) && "0000".equals(response.getResultCode())) {
                            //결과정보
