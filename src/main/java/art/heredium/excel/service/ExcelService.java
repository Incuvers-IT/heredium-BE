...
public class ExcelService {
    ...
    private String getStringCellValue(FormulaEvaluator evaluator, Row row, int index, String defaultValue) {
        Cell cell = row.getCell(index);
    ...
    private Integer getNumericCellValue(FormulaEvaluator evaluator, Row row, int index, Integer defaultValue) {
        Cell cell = row.getCell(index);
        if (cell == null) return defaultValue;
        String str;
        if (cell.getCellType() == CellType.FORMULA) {
        ...
        str = str.replace("\u00a0", "");
        if (StringUtils.isBlank(str) || str.equals("\"\"") || str.equals("#REF!") || str.equals("-") || str.equals("별도상담"))
            return defaultValue;
        return (int) Double.parseDouble(str.replaceAll(",", ""));
    }
