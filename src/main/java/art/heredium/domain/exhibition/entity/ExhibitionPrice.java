...
public class ExhibitionPrice implements Serializable {
    ...
    public ExhibitionPrice(PostAdminExhibitionRequest.Price dto, Exhibition exhibition) {
        ...
        this.discounts = dto.getDiscounts().stream().map(discount -> new ExhibitionPriceDiscount(discount, this)).collect(Collectors.toList());
    }
    ...
    public void update(PostAdminExhibitionRequest.Price dto) {
        ...
        dto.getDiscounts().forEach(dtoPriceDiscounts -> {
            ExhibitionPriceDiscount up = this.getDiscounts().stream().filter(price -> dtoPriceDiscounts.getId() != null && price.getId().intValue() == dtoPriceDiscounts.getId()).findAny().orElse(null);
            if (up == null) {
