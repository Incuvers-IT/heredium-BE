...
public class ExhibitionRound implements TicketInfo, Serializable {
    ...
    public boolean isClose() {
        ...
        LocalDateTime ticketLimitDate = this.getStartDate().isBefore(this.getEndDate().minus(30, ChronoUnit.MINUTES))
    ...
    public String getTicketId() {
        return TicketKindType.EXHIBITION.getDesc() + "-" + this.getExhibition().getId() + "-" + this.getId();
    }
    ...
    public TicketCreateInfo getTicketCreateInfo(TicketOrderInfo dto) {
        ...
        List<TicketCreateInfo.Price> prices = entity.getPrices()
                .stream()
                .filter(price -> dto.getPrices().stream().anyMatch(dp -> dp.getId().longValue() == price.getId().longValue()))
                .map(price -> {
                    TicketOrderInfo.Price dtoPrice = dto.getPrices()
                            .stream()
                    ...
                    String note = null;
                    ExhibitionPriceDiscount discount = price.getDiscounts()
                            .stream()
                    ...
                    return new TicketCreateInfo.Price(price.getType(), dtoPrice.getNumber(), price.getPrice(), discountPrice, note);
                })
    ...
    public TicketCreateInfo getTicketCreateInfo(PostAdminTicketGroupRequest dto) {
        ...
        TicketCreateInfo.Price price = new TicketCreateInfo.Price(ProjectPriceType.GROUP.getDesc(), dto.getNumber(), dto.getPrice(), null, null);
        prices.add(price);
