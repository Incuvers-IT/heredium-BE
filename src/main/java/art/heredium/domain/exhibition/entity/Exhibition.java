  ...
  public Exhibition(PostAdminExhibitionRequest dto) {
    ...
    this.rounds =
        dto.getRounds().stream()
            .map(round -> new ExhibitionRound(round, this))
            .collect(Collectors.toList());
    this.prices =
        dto.getPrices().stream()
            .map(price -> new ExhibitionPrice(price, this))
            .collect(Collectors.toList());
    this.writers =
        IntStream.range(0, dto.getWriters().size())
            .mapToObj(index -> new ExhibitionWriter(dto.getWriters().get(index), this, index))
            .collect(Collectors.toList());
  }
