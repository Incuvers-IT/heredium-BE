...
@ToString
//전시
public class Exhibition extends BaseEntity implements TicketInviteInfo, ProjectInfo, Serializable {
    ...
    private List<ExhibitionRound> rounds = new ArrayList<>();

    @OneToMany(mappedBy = "exhibition", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<ExhibitionPrice> prices = new ArrayList<>();

    @OneToMany(mappedBy = "exhibition", cascade = CascadeType.ALL, orphanRemoval = true)
    @OrderBy("order ASC")
    private List<ExhibitionWriter> writers = new ArrayList<>();

    public Exhibition(PostAdminExhibitionRequest dto) {
        ...
        this.rounds = dto.getRounds().stream().map(round -> new ExhibitionRound(round, this)).collect(Collectors.toList());
        this.prices = dto.getPrices().stream().map(price -> new ExhibitionPrice(price, this)).collect(Collectors.toList());
        this.writers = IntStream.range(0, dto.getWriters().size()).mapToObj(index -> new ExhibitionWriter(dto.getWriters().get(index), this, index)).collect(Collectors.toList());
    }

    public void update(PostAdminExhibitionRequest dto) {
        dto.validate(this);
        this.thumbnail = dto.getThumbnail();
        this.detailImage = dto.getDetailImage();
        this.title = dto.getTitle();
        ...
        this.getRounds().removeIf(round -> dto.getRounds().stream().noneMatch(dtoRound -> dtoRound.getId() != null && dtoRound.getId() == round.getId().intValue()));
        dto.getRounds().forEach(dtoRound -> {
            ExhibitionRound up = this.getRounds().stream().filter(round -> dtoRound.getId() != null && round.getId().intValue() == dtoRound.getId()).findAny().orElse(null);
            if (up == null) {
                this.addRound(new ExhibitionRound(dtoRound, this));
            } else {
                up.update(dtoRound);
            }
        });

        this.getPrices().removeIf(price -> dto.getPrices().stream().noneMatch(dtoPrice -> dtoPrice.getId() != null && dtoPrice.getId() == price.getId().intValue()));
        dto.getPrices().forEach(dtoPrice -> {
            ExhibitionPrice up = this.getPrices().stream().filter(price -> dtoPrice.getId() != null && price.getId().intValue() == dtoPrice.getId()).findAny().orElse(null);
            if (up == null) {
                this.addPrice(new ExhibitionPrice(dtoPrice, this));
            } else {
                up.update(dtoPrice);
            }
        });

        this.getWriters().removeIf(writer -> dto.getWriters().stream().noneMatch(dtoWriter -> dtoWriter.getId() != null && dtoWriter.getId() == writer.getId().intValue()));
        IntStream.range(0, dto.getWriters().size()).forEach(index -> {
            PostAdminExhibitionRequest.Writer dtoWriter = dto.getWriters().get(index);
            ExhibitionWriter up = this.getWriters().stream().filter(writer -> dtoWriter.getId() != null && writer.getId().intValue() == dtoWriter.getId()).findAny().orElse(null);
            if (up == null) {
                this.addWriter(new ExhibitionWriter(dtoWriter, this, index));
            } else {
    ...
    public void addRound(ExhibitionRound entity) {
        this.rounds.add(entity);
    }

    public void addPrice(ExhibitionPrice entity) {
        this.prices.add(entity);
    }

    public void addWriter(ExhibitionWriter entity) {
        this.writers.add(entity);
    }

    public List<String> getRemoveFile(PostAdminExhibitionRequest dto) {
        List<Storage> entityStorages = new ArrayList<>();
        entityStorages.add(this.getThumbnail());
        entityStorages.add(this.getDetailImage());
        this.getWriters().forEach(writer -> writer.getInfos().forEach(writerInfo -> entityStorages.add(writerInfo.getThumbnail())));
        List<Storage> dtoStorages = new ArrayList<>();
        dtoStorages.add(dto.getThumbnail());
        dtoStorages.add(dto.getDetailImage());
        dto.getWriters().forEach(writer -> writer.getInfos().forEach(writerInfo -> dtoStorages.add(writerInfo.getThumbnail())));
    ...
    public void applyTempFile(CloudStorage cloudStorage) {
        ...
        this.getWriters().forEach(writer -> {
            writer.getInfos().forEach(info -> {
    ...
    public Log createInsertLog(Admin admin) {
        return new Log(admin, this.title, this.toString(), LogType.EXHIBITION, LogAction.INSERT);
    }

    public Log createUpdateLog(Admin admin) {
        return new Log(admin, this.title, this.toString(), LogType.EXHIBITION, LogAction.UPDATE);
    }

    public Log createDeleteLog(Admin admin) {
        return new Log(admin, this.title, this.toString(), LogType.EXHIBITION, LogAction.DELETE);
    }

    public String getFileFolderPath() {
        return FilePathType.EXHIBITION.getPath() + "/" + this.id;
    }
    ...
    public TicketInviteCreateInfo getTicketCreateInfo(PostAdminTicketInviteRequest dto) {
        ...
        info.setKind(TicketKindType.EXHIBITION);
