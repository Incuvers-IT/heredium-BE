...
@Getter
@Setter
public class PostAdminExhibitionRequest {
    ...
    @NotNull
    private List<PostAdminExhibitionRequest.@Valid Round> rounds = new ArrayList<>();
    @NotNull
    private List<PostAdminExhibitionRequest.@Valid Price> prices = new ArrayList<>();
    @NotNull
    private List<PostAdminExhibitionRequest.@Valid Writer> writers = new ArrayList<>();
    ...
    public static class Price {
        ...
        private List<PostAdminExhibitionRequest.Price.@Valid PriceDiscount> discounts = new ArrayList<>();
    ...
    public static class Writer {
        ...
        @Length(max = 30)
        private String name;
        private List<PostAdminExhibitionRequest.Writer.@Valid WriterInfo> infos = new ArrayList<>();
    ...
    public void validate(CloudStorage cloudStorage) {
        ...
        this.getWriters().forEach(writer -> writer.getInfos().forEach(info -> validateImage(cloudStorage, info.getThumbnail())));
    ...
    public void validate(Exhibition entity) {
        LocalDateTime bookingStartDate = entity.getBookingStartDate();
        LocalDateTime bookingEndDate = entity.getBookingEndDate();
        long originRoundCount = entity.getRounds().stream().filter(round -> round.getStartDate().isBefore(bookingEndDate)).count();
        long dtoRoundCount = this.getRounds().stream().filter(round -> round.getStartDate().isBefore(bookingEndDate)).count();
        if (originRoundCount > dtoRoundCount) {
            throw new ApiException(ErrorCode.BAD_VALID, "지난 회차와 예매가능한 회차는 삭제가 불가능");
        }
        if (originRoundCount < dtoRoundCount) {
            throw new ApiException(ErrorCode.BAD_VALID, "지난 회차와 예매가능한 회차는 추가가 불가능");
        }
        ...
        if (state == ProjectStateType.BOOKING || state == ProjectStateType.PROGRESS || state == ProjectStateType.TERMINATION) {
            this.getRounds().forEach(round -> {
                ExhibitionRound originRound = entity.getRounds().stream().filter(r -> round.getId() != null && r.getId().longValue() == round.getId()).findAny().orElse(null);
                if (originRound != null) {
                    if (!originRound.getStartDate().isBefore(bookingStartDate) && !originRound.getStartDate().isAfter(bookingEndDate)) {
                        if (!round.startDate.isEqual(originRound.getStartDate()) || !round.endDate.isEqual(originRound.getEndDate())) {
