...
public class ExhibitionRepositoryImpl implements ExhibitionRepositoryQueryDsl {
  ...
  public Page<GetAdminExhibitionResponse> search(GetAdminExhibitionRequest dto, Pageable pageable) {
    ...
    List<GetAdminExhibitionResponse> result =
        total > 0
            ? queryFactory
                .select(
                    new QGetAdminExhibitionResponse(
                        QExhibition.exhibition.id,
                        ...
                        ExpressionUtils.as(
                            JPAExpressions.select(QTicket.ticket.number.sum().coalesce(0))
                                ...
                                .where(
                                    totalBookingFilter(
                                        TicketKindType.EXHIBITION, QExhibition.exhibition.id)),
                            totalBooking),
                ...
                .where(searchFilter(dto))
                .orderBy(
                    QExhibition.exhibition.startDate.desc(),
                    QExhibition.exhibition.endDate.desc(),
                    QExhibition.exhibition.lastModifiedDate.desc())
  ...
  private JPAQuery<Exhibition> getSearchQuery(GetUserExhibitionRequest dto) {
    ...
    NumberExpression<Integer> state =
        new CaseBuilder()
            .when(
                now.before(QExhibition.exhibition.startDate)
                    .and(now.before(QExhibition.exhibition.bookingDate)))
    ...
    JPAQuery<Exhibition> searchQuery =
        queryFactory
  ...
  private BooleanBuilder searchFilter(GetAdminExhibitionRequest dto) {
    ...
    if (dto.getSearchDateType().equals(GetAdminExhibitionRequest.SearchDateType.CREATED_DATE)) {
      ...
      builder.and(createdDateLoe(dto.getEndDate()));
        ...
        .equals(GetAdminExhibitionRequest.SearchDateType.LAST_MODIFIED_DATE)) {
  ...
  private BooleanExpression hallJsonSearch(HallType value) {
    return value != null
        ? Expressions.stringTemplate(
                "JSON_SEARCH({0}, {1}, {2})", QExhibition.exhibition.halls, "one", value.name())
            .isNotNull()
        : null;
  }
  ...
  private BooleanExpression lastModifiedNameContain(String value) {
    return !StringUtils.isBlank(value)
        ? QExhibition.exhibition.lastModifiedName.contains(value)
        : null;
  }
