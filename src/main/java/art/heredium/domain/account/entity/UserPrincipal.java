...
public class UserPrincipal implements UserDetails {
  ...
  public UserPrincipal(Account account) {
    ...
    if (accountInfo != null) {
      ...
      this.authorities =
          Collections.singleton(new SimpleGrantedAuthority(accountInfo.getAuth().getRole()));
    } else if (sleeperInfo != null) {
      this.isSleeper = true;
      this.auth = sleeperInfo.getAuth();
      this.name = sleeperInfo.getName();
      this.authorities =
          Collections.singleton(new SimpleGrantedAuthority(sleeperInfo.getAuth().getRole()));
    }
  }
  ...
  public UserPrincipal(Admin admin) {
    ...
    this.authorities =
        Collections.singleton(new SimpleGrantedAuthority(admin.getAdminInfo().getAuth().getRole()));
  }
}
