...
public interface AccountRepository extends JpaRepository<Account, Long>, AccountRepositoryQueryDsl {
    ...
    Long countSignUp(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

    boolean existsByEmailAndAccountInfo_PhoneAndProviderType(String email, String accountInfo_phone, OAuth2Provider providerType);

    @Query("SELECT a FROM Account a INNER JOIN FETCH SleeperInfo si ON a.id = si.account.id WHERE FUNCTION('DATEDIFF', CURRENT_TIMESTAMP, si.sleepDate) >= :day")
    List<Account> findToTerminate(@Param("day") int day);

    @Query("SELECT a FROM Account a INNER JOIN FETCH AccountInfo ai ON a.id = ai.id WHERE ai.phone = :phone")
