...
public class AccountRepositoryImpl implements AccountRepositoryQueryDsl {
    ...
    @Override
    public Page<GetAccountTicketGroupResponse> search(GetAccountTicketGroupRequest dto, Pageable pageable) {
        ...
        List<GetAccountTicketGroupResponse> result = total > 0 ? queryFactory
                .select(new QGetAccountTicketGroupResponse(
                        QAccount.account.id,
    ...
    private JPQLQuery<Long> selectVisitCount() {
        return JPAExpressions.select(Wildcard.count)
                .from(QTicket.ticket)
                .where(QTicket.ticket.account.id.eq(QAccount.account.id), QTicket.ticket.kind.eq(TicketKindType.EXHIBITION), QTicket.ticket.type.ne(TicketType.INVITE), QTicket.ticket.state.eq(TicketStateType.USED));
    }

    @Override
    public Page<GetAccountTicketInviteResponse> search(GetAccountTicketInviteRequest dto, Pageable pageable) {
        NumberPath<Long> visitCount = Expressions.numberPath(Long.class, "visitCount");
        ...
        JPQLQuery<Long> selectInviteCount = JPAExpressions.select(Wildcard.count)
                .from(QTicket.ticket)
                .where(QTicket.ticket.account.id.eq(QAccount.account.id), QTicket.ticket.kind.eq(TicketKindType.EXHIBITION), QTicket.ticket.type.eq(TicketType.INVITE));
        ...
        Long total = queryFactory
                ...
                .fetch()
                .get(0);
        List<GetAccountTicketInviteResponse> result = total > 0 ? queryFactory
                .select(new QGetAccountTicketInviteResponse(
                        QAccount.account.id,
                        QAccount.account.email,
                        QAccountInfo.accountInfo.name,
                        QAccountInfo.accountInfo.phone,
    ...
    public Page<GetAdminAccountResponse> search(GetAdminAccountRequest dto, Pageable pageable) {
        ...
        List<GetAdminAccountResponse> result = total > 0 ? getGetAdminAccountQuery(dto)
                .offset(pageable.getOffset())
                .limit(pageable.getPageSize())
                .fetch() : new ArrayList<>();
    ...
    private JPAQuery<GetAdminAccountResponse> getGetAdminAccountQuery(GetAdminAccountRequest dto) {
        NumberPath<Long> visitCount = Expressions.numberPath(Long.class, "visitCount");
        return queryFactory
                .select(new QGetAdminAccountResponse(
                        QAccount.account.id,
                        QAccount.account.email,
                        QAccountInfo.accountInfo.name,
                        QAccountInfo.accountInfo.phone,
    ...
    public Page<GetAdminSleeperResponse> search(GetAdminSleeperRequest dto, Pageable pageable) {
        Long total = queryFactory
                .select(Wildcard.count)
                .from(QAccount.account)
                .innerJoin(QSleeperInfo.sleeperInfo).on(QAccount.account.id.eq(QSleeperInfo.sleeperInfo.id))
                .where(searchFilter(dto))
                .fetch()
                .get(0);

        List<GetAdminSleeperResponse> result = total > 0 ? getGetAdminAccountQuery(dto)
                .offset(pageable.getOffset())
                .limit(pageable.getPageSize())
                .fetch() : new ArrayList<>();
    ...
    private JPAQuery<GetAdminSleeperResponse> getGetAdminAccountQuery(GetAdminSleeperRequest dto) {
        NumberPath<Long> visitCount = Expressions.numberPath(Long.class, "visitCount");
        return queryFactory
                .select(new QGetAdminSleeperResponse(
                        QAccount.account.id,
                        QAccount.account.email,
    ...
    private BooleanBuilder searchFilter(GetAdminAccountRequest dto) {
        ...
        } else if (dto.getSearchDateType().equals(GetAdminAccountRequest.SearchDateType.LAST_LOGIN_DATE)) {
    ...
    private BooleanBuilder searchFilter(GetAccountTicketInviteRequest dto, JPQLQuery<Long> visitCount, JPQLQuery<Long> inviteCount) {
        BooleanBuilder builder = new BooleanBuilder();
        if (dto.getSearchDateType().equals(GetAccountTicketInviteRequest.SearchDateType.CREATED_DATE)) {
            builder.and(createdDateGoe(dto.getStartDate()));
            builder.and(createdDateLoe(dto.getEndDate()));
        } else if (dto.getSearchDateType().equals(GetAccountTicketInviteRequest.SearchDateType.LAST_LOGIN_DATE)) {
            builder.and(lastLoginDateGoe(dto.getStartDate()));
            builder.and(lastLoginDateLoe(dto.getEndDate()));
        }
