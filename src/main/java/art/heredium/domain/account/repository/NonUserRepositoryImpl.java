...
public class NonUserRepositoryImpl implements NonUserRepositoryQueryDsl {
  ...
  public Page<GetAdminHanaBankUserResponse> search(GetAdminHanaBankRequest dto, Pageable pageable) {
    Long total =
        queryFactory.select(Wildcard.count).from(nonUser).where(searchFilter(dto)).fetch().get(0);

    List<GetAdminHanaBankUserResponse> result =
        total > 0
  ...
  private JPQLQuery<Long> selectVisitCount() {
    return JPAExpressions.select(Wildcard.count)
        .from(QTicket.ticket)
        .where(
            QTicket.ticket.nonUser.id.eq(nonUser.id),
  ...
  private JPAQuery<GetAdminHanaBankUserResponse> getGetAdminAccountQuery(
      GetAdminHanaBankRequest dto) {
    NumberPath<Long> visitCount = Expressions.numberPath(Long.class, "visitCount");
    return queryFactory
        .select(
            new QGetAdminHanaBankUserResponse(
                nonUser.id,
                nonUser.name,
