...
@ToString
//프로그램
public class Program extends BaseEntity implements TicketInviteInfo, ProjectInfo, Serializable {
    private static final long serialVersionUID = 2101072942139474522L;
    ...
    @OneToMany(mappedBy = "program", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<ProgramPrice> prices = new ArrayList<>();

    @OneToMany(mappedBy = "program", cascade = CascadeType.ALL, orphanRemoval = true)
    @OrderBy("order ASC")
    private List<ProgramWriter> writers = new ArrayList<>();

    public Program(PostAdminProgramRequest dto) {
        ...
        this.rounds = dto.getRounds().stream().map(round -> new ProgramRound(round, this)).collect(Collectors.toList());
        this.prices = dto.getPrices().stream().map(price -> new ProgramPrice(price, this)).collect(Collectors.toList());
        this.writers = IntStream.range(0, dto.getWriters().size()).mapToObj(index -> new ProgramWriter(dto.getWriters().get(index), this, index)).collect(Collectors.toList());
    }

    public void update(PostAdminProgramRequest dto) {
        dto.validate(this);
        this.thumbnail = dto.getThumbnail();
        this.detailImage = dto.getDetailImage();
        this.title = dto.getTitle();
        ...
        dto.getRounds().forEach(dtoRound -> {
            ProgramRound up = this.getRounds().stream().filter(round -> dtoRound.getId() != null && round.getId().intValue() == dtoRound.getId()).findAny().orElse(null);
            if (up == null) {
                this.addRound(new ProgramRound(dtoRound, this));
            } else {
                up.update(dtoRound);
            }
        });
        ...
        dto.getPrices().forEach(dtoPrice -> {
            ProgramPrice up = this.getPrices().stream().filter(price -> dtoPrice.getId() != null && price.getId().intValue() == dtoPrice.getId()).findAny().orElse(null);
            if (up == null) {
                this.addPrice(new ProgramPrice(dtoPrice, this));
            } else {
                up.update(dtoPrice);
            }
        });
        ...
        IntStream.range(0, dto.getWriters().size()).forEach(index -> {
            PostAdminProgramRequest.Writer dtoWriter = dto.getWriters().get(index);
            ProgramWriter up = this.getWriters().stream().filter(writer -> dtoWriter.getId() != null && writer.getId().intValue() == dtoWriter.getId()).findAny().orElse(null);
            if (up == null) {
                this.addWriter(new ProgramWriter(dtoWriter, this, index));
            } else {
    ...
    public void addRound(ProgramRound entity) {
        this.rounds.add(entity);
    }

    public void addPrice(ProgramPrice entity) {
        this.prices.add(entity);
    }

    public void addWriter(ProgramWriter entity) {
        this.writers.add(entity);
    }

    public List<String> getRemoveFile(PostAdminProgramRequest dto) {
    ...
    public Log createInsertLog(Admin admin) {
        return new Log(admin, this.title, this.toString(), LogType.PROGRAM, LogAction.INSERT);
    }

    public Log createUpdateLog(Admin admin) {
        return new Log(admin, this.title, this.toString(), LogType.PROGRAM, LogAction.UPDATE);
    }

    public Log createDeleteLog(Admin admin) {
        return new Log(admin, this.title, this.toString(), LogType.PROGRAM, LogAction.DELETE);
    }

    public String getFileFolderPath() {
        return FilePathType.PROGRAM.getPath() + "/" + this.id;
    }
    ...
    public TicketInviteCreateInfo getTicketCreateInfo(PostAdminTicketInviteRequest dto) {
        ...
        info.setKind(TicketKindType.PROGRAM);
