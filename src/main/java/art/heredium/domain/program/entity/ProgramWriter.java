...
public class ProgramWriter implements Serializable {
    ...
    public ProgramWriter(PostAdminProgramRequest.Writer dto, Program program, long order) {
        ...
        this.order = order;
        this.infos = IntStream.range(0, dto.getInfos().size()).mapToObj(index -> new ProgramWriterInfo(dto.getInfos().get(index), this, index)).collect(Collectors.toList());
    }
    ...
    public void update(PostAdminProgramRequest.Writer dto, long order) {
        ...
        IntStream.range(0, dto.getInfos().size()).forEach(index -> {
            ...
            ProgramWriterInfo up = this.getInfos().stream().filter(writer -> dtoInfo.getId() != null && writer.getId().intValue() == dtoInfo.getId()).findAny().orElse(null);
            if (up == null) {
