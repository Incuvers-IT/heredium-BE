...
public class ProgramRepositoryImpl implements ProgramRepositoryQueryDsl {
    ...
    public Page<GetAdminProgramResponse> search(GetAdminProgramRequest dto, Pageable pageable) {
        ...
        List<GetAdminProgramResponse> result = total > 0 ? queryFactory
                .select(new QGetAdminProgramResponse(QProgram.program.id,
                        ...
                        ExpressionUtils.as(JPAExpressions.select(QTicket.ticket.number.sum().coalesce(0))
                                        .from(QTicket.ticket)
                                        .where(totalBookingFilter(TicketKindType.PROGRAM, QProgram.program.id)),
                                totalBooking),
                ...
                .from(QProgram.program)
                .where(searchFilter(dto))
                .orderBy(QProgram.program.startDate.desc(), QProgram.program.endDate.desc(), QProgram.program.lastModifiedDate.desc())
    ...
    public Slice<Program> search(GetUserProgramRequest dto, Pageable pageable) {
        List<Program> content = getSearchQuery(dto)
                .offset(pageable.getOffset())
    ...
    private JPAQuery<Program> getSearchQuery(GetUserProgramRequest dto) {
        ...
        NumberExpression<Integer> state = new CaseBuilder()
                .when(now.before(QProgram.program.startDate).and(now.before(QProgram.program.bookingDate)))
        ...
        JPAQuery<Program> searchQuery = queryFactory
                .selectFrom(QProgram.program)
                .where(searchFilter(dto))
                .orderBy(state.asc(), QProgram.program.startDate.desc(), QProgram.program.endDate.desc(), QProgram.program.lastModifiedDate.desc());
        return searchQuery;
    }
    ...
    private BooleanBuilder searchFilter(GetAdminProgramRequest dto) {
        ...
        } else if (dto.getSearchDateType().equals(GetAdminProgramRequest.SearchDateType.LAST_MODIFIED_DATE)) {
    ...
    private BooleanExpression hallJsonSearch(HallType value) {
        return value != null ? Expressions.stringTemplate("JSON_SEARCH({0}, {1}, {2})", QProgram.program.halls, "one", value.name()).isNotNull() : null;
    }
