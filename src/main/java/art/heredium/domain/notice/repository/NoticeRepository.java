...
public interface NoticeRepository extends JpaRepository<Notice, Long>, NoticeRepositoryQueryDsl {
  @Query(
      value =
          "WITH s AS (SELECT row_number() OVER (ORDER BY e.is_notice DESC, e.post_date DESC, e.last_modified_date DESC) AS num, e.id "
              + "           FROM notice e "
              ...
              + "SELECT e.* FROM notice e WHERE e.id = (SELECT s2.prev FROM s2 WHERE s2.id = :id)",
      nativeQuery = true)
  ...
  @Query(
      value =
          "WITH s AS (SELECT row_number() OVER (ORDER BY e.is_notice DESC, e.post_date DESC, e.last_modified_date DESC) AS num, e.id "
              ...
              + "SELECT e.* FROM notice e WHERE e.id = (SELECT s2.next FROM s2 WHERE s2.id = :id)",
      nativeQuery = true)
