...
public class CoffeeRepositoryImpl implements CoffeeRepositoryQueryDsl {
  ...
  public Page<GetAdminCoffeeResponse> search(GetAdminCoffeeRequest dto, Pageable pageable) {
    ...
    List<GetAdminCoffeeResponse> result =
        total > 0
            ? queryFactory
                .select(
                    new QGetAdminCoffeeResponse(
                        QCoffee.coffee.id,
                        ...
                        ExpressionUtils.as(
                            JPAExpressions.select(QTicket.ticket.number.sum().coalesce(0))
                                .from(QTicket.ticket)
                                .where(
                                    totalBookingFilter(TicketKindType.COFFEE, QCoffee.coffee.id)),
                            totalBooking),
                ...
                .where(searchFilter(dto))
                .orderBy(
                    QCoffee.coffee.startDate.desc(),
                    QCoffee.coffee.endDate.desc(),
                    QCoffee.coffee.lastModifiedDate.desc())
  ...
  private BooleanBuilder searchFilter(GetAdminCoffeeRequest dto) {
    ...
    if (dto.getSearchDateType().equals(GetAdminCoffeeRequest.SearchDateType.CREATED_DATE)) {
      ...
      builder.and(createdDateLoe(dto.getEndDate()));
        ...
        .equals(GetAdminCoffeeRequest.SearchDateType.LAST_MODIFIED_DATE)) {
  ...
  private BooleanExpression hallJsonSearch(HallType value) {
    return value != null
        ? Expressions.stringTemplate(
                "JSON_SEARCH({0}, {1}, {2})", QCoffee.coffee.halls, "one", value.name())
            .isNotNull()
        : null;
  }
