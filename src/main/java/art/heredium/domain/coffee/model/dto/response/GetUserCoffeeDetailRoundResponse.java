...
import art.heredium.domain.coffee.entity.CoffeePriceDiscount;
...
@Getter
@Setter
public class GetUserCoffeeDetailRoundResponse {
    private Long id;
    ...
    public GetUserCoffeeDetailRoundResponse(Coffee entity, Map<LocalDateTime, Long> ticketTotalNumber, List<LocalDate> holidays) {
        ...
        long priceCount = entity.getPrices().stream().filter(CoffeePrice::getIsEnabled).count();
        ...
        this.dates = entity.getRounds().stream()
                ...
                .stream().collect(Collectors.toMap(
                        date -> date,
                        date -> {
                            List<CoffeeRound> roundsFilterDate = entity.getRounds().stream()
        ...
        this.prices = entity.getPrices().stream()
                .filter(CoffeePrice::getIsEnabled).map(Price::new)
                .collect(Collectors.toList());
    }
