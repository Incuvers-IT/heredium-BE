...
public class GetUserCoffeeDetailRoundResponse {
  ...
  public GetUserCoffeeDetailRoundResponse(
      Coffee entity, Map<LocalDateTime, Long> ticketTotalNumber, List<LocalDate> holidays) {
        ...
        entity.getRounds().stream()
            ...
            .collect(
                Collectors.toMap(
                    date -> date,
                    date -> {
                      List<CoffeeRound> roundsFilterDate =
                          entity.getRounds().stream()
    ...
    this.prices =
        entity.getPrices().stream()
            .filter(CoffeePrice::getIsEnabled)
            .map(Price::new)
            .collect(Collectors.toList());
  }
