...
@Getter
@Setter
public class PostAdminCoffeeRequest {
    ...
    @NotNull
    private List<@Valid Round> rounds = new ArrayList<>();
    @NotNull
    private List<@Valid Price> prices = new ArrayList<>();
    @NotNull
    private List<@Valid Writer> writers = new ArrayList<>();
    ...
    public static class Price {
        ...
        private List<PostAdminCoffeeRequest.Price.@Valid PriceDiscount> discounts = new ArrayList<>();
    ...
    public void validate(Coffee entity) {
        ...
        if (state == ProjectStateType.BOOKING || state == ProjectStateType.PROGRESS || state == ProjectStateType.TERMINATION) {
            this.getRounds().forEach(round -> {
                CoffeeRound originRound = entity.getRounds().stream().filter(r -> round.getId() != null && r.getId().longValue() == round.getId()).findAny().orElse(null);
                if (originRound != null) {
