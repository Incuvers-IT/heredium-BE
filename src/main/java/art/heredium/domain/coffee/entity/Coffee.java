...
public class Coffee extends BaseEntity implements TicketInviteInfo, ProjectInfo, Serializable {
    ...
    public Coffee(PostAdminCoffeeRequest dto) {
        ...
        this.rounds = dto.getRounds().stream().map(round -> new CoffeeRound(round, this)).collect(Collectors.toList());
        this.prices = dto.getPrices().stream().map(price -> new CoffeePrice(price, this)).collect(Collectors.toList());
        this.writers = IntStream.range(0, dto.getWriters().size()).mapToObj(index -> new CoffeeWriter(dto.getWriters().get(index), this, index)).collect(Collectors.toList());
    }
    ...
    public void update(PostAdminCoffeeRequest dto) {
        ...
        dto.getRounds().forEach(dtoRound -> {
            CoffeeRound up = this.getRounds().stream().filter(round -> dtoRound.getId() != null && round.getId().intValue() == dtoRound.getId()).findAny().orElse(null);
            if (up == null) {
        ...
        dto.getPrices().forEach(dtoPrice -> {
            CoffeePrice up = this.getPrices().stream().filter(price -> dtoPrice.getId() != null && price.getId().intValue() == dtoPrice.getId()).findAny().orElse(null);
            if (up == null) {
        ...
        IntStream.range(0, dto.getWriters().size()).forEach(index -> {
            ...
            CoffeeWriter up = this.getWriters().stream().filter(writer -> dtoWriter.getId() != null && writer.getId().intValue() == dtoWriter.getId()).findAny().orElse(null);
            if (up == null) {
