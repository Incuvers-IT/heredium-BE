...
public interface EventRepository extends JpaRepository<Event, Long>, EventRepositoryQueryDsl {
  @Query(
      value =
          "WITH s AS (SELECT row_number() OVER (ORDER BY CASE "
              ...
              + "SELECT e.* FROM event e WHERE e.id = (SELECT s2.prev FROM s2 WHERE s2.id = :id)",
      nativeQuery = true)
  ...
  @Query(
      value =
          "WITH s AS (SELECT row_number() OVER (ORDER BY CASE "
              ...
              + "SELECT e.* FROM event e WHERE e.id = (SELECT s2.next FROM s2 WHERE s2.id = :id)",
      nativeQuery = true)
