...
public class StatisticsRepository {
    ...
    public GetAdminTicketStatisticsDashboardResponse dashboard(TicketKindType kind, Long kindId, String ticketPriceType) {
        return queryFactory
                .select(new QGetAdminTicketStatisticsDashboardResponse(
    ...
    public List<GetAdminStatisticsSummaryResponse.TicketPriceInfo> ticketPriceInfo(LocalDateTime startDate, LocalDateTime endDate, TicketKindType kind, Long kindId) {
        NumberPath<Long> aliasUnitPrice = Expressions.numberPath(Long.class, "unitPrice");

        NumberExpression<Long> unitPrice = new CaseBuilder()
                .when(QTicket.ticket.type.eq(TicketType.NORMAL))
                .then(QTicketPrice.ticketPrice.price)
                .otherwise(Expressions.nullExpression());

        NumberExpression<Integer> refundNumber = new CaseBuilder()
                .when(QTicket.ticket.state.in(TicketStateType.ADMIN_REFUND, TicketStateType.USER_REFUND))
                .then(QTicketPrice.ticketPrice.number)
                .otherwise(0);

        NumberExpression<Long> totalPrice = new CaseBuilder()
                .when(QTicket.ticket.state.notIn(TicketStateType.ADMIN_REFUND, TicketStateType.USER_REFUND))
                .then(QTicketPrice.ticketPrice.price.multiply(QTicketPrice.ticketPrice.number))
                .otherwise(0L);

        List<GetAdminStatisticsSummaryResponse.TicketPriceInfo> fetch = queryFactory
                .select(new QGetAdminStatisticsSummaryResponse_TicketPriceInfo(
                ...
                .from(QTicket.ticket)
                .innerJoin(QTicket.ticket.prices, QTicketPrice.ticketPrice)
                .where(ticketStartDateBetween(startDate, endDate),
                        ticketKindEq(kind),
        ...
        return fetch.stream()
                .sorted(Comparator.comparing(GetAdminStatisticsSummaryResponse.TicketPriceInfo::getUnitPrice, (a, b) -> {
                            if (a == null && b == null) return 0;
                            else if (a == null) return 1;
                            else if (b == null) return -1;
                            else return 0;
                        })
                        .thenComparing(x -> ProjectPriceType.getCodeOfDesc(x.getType()), Comparator.nullsLast(Comparator.naturalOrder())))
                .collect(Collectors.toList());
    }

    public List<ChartResponse> ticketInfoChart(LocalDateTime startDate, LocalDateTime endDate, TicketKindType kind, Long kindId) {
        StringPath label = Expressions.stringPath("label");
        NumberPath<Double> num = Expressions.numberPath(Double.class, "num");
        List<String> projectPriceTypes = Arrays.stream(ProjectPriceType.values()).map(ProjectPriceType::getDesc).collect(Collectors.toList());
        StringExpression typeCase = new CaseBuilder()
                .when(QTicketPrice.ticketPrice.type.in(projectPriceTypes))
                .then(QTicketPrice.ticketPrice.type)
                .otherwise("기타");
        return queryFactory
                .select(new QChartResponse(
                        typeCase.as(label),
    ...
    public List<ChartResponse> chart(LocalDateTime startDate, LocalDateTime endDate, TicketKindType kind, Long kindId, StatisticsType type, StatisticsDateType dateType) {

        StringPath label = Expressions.stringPath("label");
        if (type.equals(StatisticsType.SIGN_UP)) {
            return queryFactory.select(new QChartResponse(
    ...
    private BooleanBuilder chartFilter(LocalDateTime startDate, LocalDateTime endDate, TicketKindType kind, Long kindId, StatisticsType type) {
    ...
    private BooleanExpression ticketStartDateBetween(LocalDateTime startDate, LocalDateTime endDate) {
        return startDate != null && endDate != null ? QTicket.ticket.startDate.between(startDate, endDate) : null;
    }

    private BooleanExpression ticketUseDateBetween(LocalDateTime startDate, LocalDateTime endDate) {
        return startDate != null && endDate != null ? QTicket.ticket.usedDate.between(startDate, endDate) : null;
    }

    private BooleanExpression ticketCreatedDateBetween(LocalDateTime startDate, LocalDateTime endDate) {
        return startDate != null && endDate != null ? QTicket.ticket.createdDate.between(startDate, endDate) : null;
    }

    private BooleanExpression accountCratedDateBetween(LocalDateTime startDate, LocalDateTime endDate) {
        return startDate != null && endDate != null ? QAccount.account.createdDate.between(startDate, endDate) : null;
    }

    private StringTemplate getDateFormat(DateTimePath<LocalDateTime> dateField, String format) {
        return Expressions.stringTemplate("DATE_FORMAT({0}, {1})", dateField, ConstantImpl.create(format));
    }
}