...
public class GetUserCommonHomeResponse {
  ...
  public GetUserCommonHomeResponse(
      List<art.heredium.domain.popup.entity.Popup> popups,
    ...
    this.popups = popups.stream().map(Popup::new).collect(Collectors.toList());
    this.slides = slides.stream().map(Slide::new).collect(Collectors.toList());
    this.events = events.stream().map(Event::new).collect(Collectors.toList());
    this.notices = notices.stream().map(Notice::new).collect(Collectors.toList());
