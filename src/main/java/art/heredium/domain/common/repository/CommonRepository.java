...
public class CommonRepository {
    ...
    public Page<GetUserCommonSearchContentResponse> searchContent(GetUserCommonSearchRequest dto, Pageable pageable) {
        ...
        Long total = isBlinkText ? 0L : queryFactory
        ...
        List<GetUserCommonSearchContentResponse> result = total > 0 ? queryFactory.select(column)
                .from(getSearchTable(dto))
                .where(searchFilter(dto))
    ...
    private OrderSpecifier<?>[] getSearchOrder(GetUserCommonSearchRequest dto) {
        ...
        if (dto.getType().equals(GetUserCommonSearchRequest.SearchDateType.EXHIBITION)) {
            ...
            return new OrderSpecifier<?>[]{state.asc(), exhibition.startDate.desc(), exhibition.endDate.desc(), exhibition.lastModifiedDate.desc()};
        } else if (dto.getType().equals(GetUserCommonSearchRequest.SearchDateType.PROGRAM)) {
            NumberExpression<Integer> state = new CaseBuilder()
                    ...
                    .then(DateState.TERMINATION.getCode())
                    .otherwise(DateState.PROGRESS.getCode());
            return new OrderSpecifier<?>[]{state.asc(), program.startDate.desc(), program.endDate.desc(), program.lastModifiedDate.desc()};
        } else if (dto.getType().equals(GetUserCommonSearchRequest.SearchDateType.EVENT)) {
            NumberExpression<Integer> state = new CaseBuilder()
                    ...
                    .then(DateState.TERMINATION.getCode())
                    .otherwise(DateState.PROGRESS.getCode());
            return new OrderSpecifier<?>[]{state.asc(), event.startDate.desc(), event.endDate.desc(), event.lastModifiedDate.desc()};
        } else if (dto.getType().equals(GetUserCommonSearchRequest.SearchDateType.NOTICE)) {
    ...
    private QGetUserCommonSearchContentResponse getSearchColumn(GetUserCommonSearchRequest dto) {
        if (dto.getType().equals(GetUserCommonSearchRequest.SearchDateType.EXHIBITION)) {
            return new QGetUserCommonSearchContentResponse(exhibition.id, exhibition.title, exhibition.subtitle, exhibition.thumbnail, exhibition.createdDate, exhibition.startDate, exhibition.endDate);
        } else if (dto.getType().equals(GetUserCommonSearchRequest.SearchDateType.PROGRAM)) {
            return new QGetUserCommonSearchContentResponse(program.id, program.title, program.subtitle, program.thumbnail, program.createdDate, program.startDate, program.endDate);
        } else if (dto.getType().equals(GetUserCommonSearchRequest.SearchDateType.EVENT)) {
            return new QGetUserCommonSearchContentResponse(event.id, event.title, event.thumbnail, event.createdDate, event.startDate, event.endDate);
        } else if (dto.getType().equals(GetUserCommonSearchRequest.SearchDateType.NOTICE)) {
            return new QGetUserCommonSearchContentResponse(notice.id, notice.title, notice.isNotice, notice.postDate);
        }
        return null;
    }
