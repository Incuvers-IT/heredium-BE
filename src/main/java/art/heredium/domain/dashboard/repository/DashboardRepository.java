...
public class DashboardRepository {
  ...
  public AdminDashBoardResponse.Count dashboard(LocalDate now) {
    Integer visitCount =
        queryFactory
            ...
            .where(
                ticketUseDateEq(now),
                ticketKindNe(TicketKindType.COFFEE),
                ticketStateEq(TicketStateType.USED))
            .fetchOne();
    ...
    return new AdminDashBoardResponse.Count(
        visitCount, totalCount, saleCount, salePrice, refundCount, refundPrice, newRegister);
  }
  ...
  private BooleanBuilder refundFilter() {
    BooleanBuilder builder = new BooleanBuilder();
    builder.and(
        ticketStateEq(TicketStateType.ADMIN_REFUND).or(ticketStateEq(TicketStateType.USER_REFUND)));
    builder.and(ticketTypeNe(TicketType.INVITE));
    return builder;
  }

  private BooleanExpression ticketStartDateEq(LocalDate now) {
    return Expressions.dateTemplate(Date.class, "DATE({0})", QTicket.ticket.startDate)
        .eq(Date.valueOf(now));
  }

  private BooleanExpression ticketUseDateEq(LocalDate now) {
    return Expressions.dateTemplate(Date.class, "DATE({0})", QTicket.ticket.usedDate)
        .eq(Date.valueOf(now));
  }

  private BooleanExpression ticketCreateDateEq(LocalDate now) {
    return Expressions.dateTemplate(Date.class, "DATE({0})", QTicket.ticket.createdDate)
        .eq(Date.valueOf(now));
  }

  private BooleanExpression accountCreatedDateEq(LocalDate now) {
    return Expressions.dateTemplate(Date.class, "DATE({0})", QAccount.account.createdDate)
        .eq(Date.valueOf(now));
  }
