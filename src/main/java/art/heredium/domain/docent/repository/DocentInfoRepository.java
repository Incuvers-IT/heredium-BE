...
public interface DocentInfoRepository extends JpaRepository<DocentInfo, Long> {
  ...
  @Query(
      "SELECT di FROM DocentInfo di INNER JOIN Docent d ON d.id = di.docent.id WHERE d.id = :id AND d.startDate <= CURRENT_TIMESTAMP AND d.endDate >= CURRENT_TIMESTAMP AND d.isEnabled IS TRUE ORDER BY di.order ASC")
  List<DocentInfo> findPostingInfos(@Param("id") Long id);

  @Query(
      value =
          "WITH s AS (SELECT row_number() OVER (ORDER BY di.orders ASC) AS num, di.id "
              + "           FROM docent_info di "
              ...
              + "SELECT di.* FROM docent_info di WHERE di.id = (SELECT s2.recordId FROM s2 WHERE s2.id = :id)",
      nativeQuery = true)
