...
public class TicketRepositoryImpl implements TicketRepositoryQueryDsl {
    ...
    public Page<Ticket> search(GetAdminTicketRequest dto, Pageable pageable) {
        ...
        List<Ticket> result = total > 0 ? getAdminTicketQuery(dto)
    ...
    public List<Ticket> search(GetAdminTicketRequest dto) {
        return getAdminTicketQuery(dto)
                .fetch();
    }

    @Override
    public Page<GetUserMemberTicketResponse> findByMember(Long accountId, List<TicketKindType> kinds, Integer year, Pageable pageable) {
    ...
    @Override
    public Page<GetUserMemberTicketResponse> findByNonUser(GetUserNonUserTicketRequest dto, Pageable pageable) {
    ...
    private PageImpl<GetUserMemberTicketResponse> getGetUserMemberTicketResponses(Pageable pageable, BooleanBuilder byNonUserFilter) {
        JPAQuery<?> jpaQuery = queryFactory
                .from(ticket)
                .leftJoin(exhibition).on(ticket.kindId.eq(exhibition.id).and(ticket.kind.eq(TicketKindType.EXHIBITION)))
                .leftJoin(program).on(ticket.kindId.eq(program.id).and(ticket.kind.eq(TicketKindType.PROGRAM)))
                .leftJoin(coffee).on(ticket.kindId.eq(coffee.id).and(ticket.kind.eq(TicketKindType.COFFEE)))
        ...
        List<GetUserMemberTicketResponse> result = total > 0 ? jpaQuery
                .select(new QGetUserMemberTicketResponse(
                        ticket.id,
                        Expressions.template(Storage.class, "COALESCE({0}, {1}, {2})", exhibition.thumbnail, program.thumbnail, coffee.thumbnail),
                        ticket.title,
                        ticket.uuid,
                        ticket.kind,
                        ticket.state,
    ...
    public List<GetUserMemberTicketEnabledResponse> ticketByAccountAndEnabled(Long id) {
        ...
        NumberExpression<Integer> typeSort = new CaseBuilder()
                .when(ticket.type.eq(TicketType.INVITE))
                .then(1)
                .otherwise(0);
        return queryFactory
                .from(ticket)
                .select(new QGetUserMemberTicketEnabledResponse(
                        ticket.id,
                        ticket.title,
                        ticket.kind,
                        ticket.type,
    ...
    private JPQLQuery<String> getPriceInfo() {
        return JPAExpressions.select(Expressions.stringTemplate("GROUP_CONCAT_SEPARATOR({0}, {1})", Expressions.stringTemplate("CONCAT({0}, '=-=-=-=', {1})", QTicketPrice.ticketPrice.type, QTicketPrice.ticketPrice.number), "=-,-="))
    ...
    private BooleanBuilder findByMemberFilter(Long accountId, List<TicketKindType> kinds, Integer year) {
        BooleanBuilder builder = new BooleanBuilder();
    ...
    private BooleanExpression ticketToday() {
        return Expressions.numberTemplate(Integer.class, "DATEDIFF({0}, {1})", DateTimeExpression.currentTimestamp(LocalDateTime.class), ticket.startDate).eq(0).or(ticket.type.eq(TicketType.INVITE));
    }

    private BooleanExpression createdDateYearEq(Integer value) {
        return value != null ? Expressions.numberTemplate(Integer.class, "YEAR({0})", ticket.createdDate).eq(value) : null;
    }
}