...
public class Ticket implements Serializable {
    ...
    public Ticket(List<TicketPrice> prices,
        ...
        this.originPrice = prices.stream().mapToLong(price -> price.getNumber() * price.getOriginPrice()).sum();
    ...
    public Ticket(PostAdminTicketGroupRequest dto,
                  TicketCreateInfo info,
                  Account account,
                  Admin admin) {
        this.account = account;
        ...
        this.state = TicketStateType.PAYMENT;
        this.prices.add(new TicketPrice(this, ProjectPriceType.GROUP.getDesc(), this.getNumber(), this.getPrice() / this.getNumber(), this.getPrice() / this.getNumber(), null));
        this.logs.add(new TicketLog(this, admin, admin.getAdminInfo().getName(), null, this.getState()));
    }

    public Ticket(TicketInviteCreateInfo info,
                  Account account,
                  Admin admin) {
        this.account = account;
        ...
        this.prices.add(new TicketPrice(this, ProjectPriceType.INVITE.getDesc(), this.getNumber(), 0L, 0L, null));
        this.logs.add(new TicketLog(this, admin, admin.getAdminInfo().getName(), null, this.getState()));
    }
    ...
    public void updateState(UserPrincipal userPrincipal, TicketStateType state) {
        this.logs.add(new TicketLog(this, userPrincipal.getAdmin(), userPrincipal.getName(), this.getState(), state));
        this.state = state;
    }
    ...
    public boolean isRefund() {
        return (this.getState().equals(TicketStateType.ADMIN_REFUND) || this.getState().equals(TicketStateType.USER_REFUND));
    }

    public boolean isCanUse() {
        return this.getState().equals(TicketStateType.PAYMENT) || this.getState().equals(TicketStateType.ISSUANCE);
    }

    public boolean isNormalPayment() {
        return this.getState().equals(TicketStateType.PAYMENT) && this.getType().equals(TicketType.NORMAL);
    }
    ...
    public Map<String, String> getMailParam(HerediumProperties herediumProperties) {
        ...
        param.put("startDate", this.getStartDate().format(DateTimeFormatter.ofPattern("yyyy-MM-dd (E) HH:mm")));
        ...
        param.put("info", this.getPrices().stream()
                .map(ticketPrice -> String.format("%s %d매", ticketPrice.getType(), ticketPrice.getNumber()))
