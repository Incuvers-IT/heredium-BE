...
public class HanaParams {
  ...
  public static Map<String, String> encrypt(String key, String salt, String nonceParam, String data)
    ...
    {
      final byte[] random =
          hmacSha256(hashKey, (salt + "|#" + nonce).getBytes(StandardCharsets.UTF_8));
    ...
    final byte[] cipher =
        aes256CbcP5encrypt(cipherKey, cipherIv, data.getBytes(StandardCharsets.UTF_8));
  ...
  public static String parse(
      String key, String salt, String messageParam, String macParam, String nonceParam)
      throws HanaParamsException {
    final byte[] cipher;
    try {
  ...
  private static byte[] aes256CbcP5encrypt(byte[] key, byte[] iv, byte[] data)
      throws HanaParamsException {
  ...
  private static byte[] aes256CbcP5Decrypt(byte[] key, byte[] iv, byte[] encrypted)
      throws HanaParamsException {
    assert key.length == AES256_KEY_SIZE;
    assert iv.length == AES_BLOCK_SIZE;
