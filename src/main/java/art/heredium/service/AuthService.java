...
public class AuthService {
    ...
    public PostLoginResponse login(HttpServletResponse response, PostLoginRequest dto, boolean isAdmin) {
        ...
        if (isAdmin) {
            ...
            Account account = accountRepository.findByEmailEqualsAndProviderType(dto.getEmail(), OAuth2Provider.EMAIL);
            if (account == null) {
                throw new ApiException(ErrorCode.USER_NOT_FOUND);
            }
        ...
        String refreshTokenName = isAdmin ? appProperties.getAuth().getAdminRefreshTokenName() : appProperties.getAuth().getRefreshTokenName();
    ...
    public String refreshToken(HttpServletRequest request, HttpServletResponse response, String accessToken) {
        ...
        String refreshTokenName = isAdmin ? appProperties.getAuth().getAdminRefreshTokenName() : appProperties.getAuth().getRefreshTokenName();
        // refresh token
        String refreshToken = CookieUtil.getCookie(request, refreshTokenName)
                .map(Cookie::getValue)
                .orElse((null));
        ...
        AuthToken newAccessToken = authTokenProvider.createAuthToken(userId, accessTokenAuth, new Date(now.getTime() + appProperties.getAuth().getAccessTokenExpiry().toMillis()));
        if (validTime > appProperties.getAuth().getRefreshTokenMaintain().toMillis()) {
            return newAccessToken.getToken();
        }
        // refresh 토큰 기간이 3일 이하로 남은 경우, refresh 토큰 갱신
        AuthToken newRefreshToken = authTokenProvider.createAuthToken(userId, accessTokenAuth, new Date(now.getTime() + appProperties.getAuth().getRefreshTokenExpiry().toMillis()));
        jwtRedisUtil.setDataExpire(newRefreshToken.getToken(), userId, appProperties.getAuth().getRefreshTokenExpiry().getSeconds());
    ...
    private void updateLoginDate(boolean isAdmin, String userId) {
        if (isAdmin) {
            adminRepository.findById(Long.valueOf(userId))
    ...
    public Object getRefreshToken(HttpServletRequest request, HttpServletResponse response, String accessToken) {
