...
public class TicketService {
        ...
        if (state.equals(TicketStateType.ADMIN_REFUND)) {
            ...
            if (entity.getType() == TicketType.NORMAL) {
                ...
                if (!StringUtils.isBlank(entity.getPhone())) {
                    alimTalk.sendAlimTalk(entity.getPhone(), entity.getMailParam(herediumProperties), AlimTalkTemplate.TICKET_REFUND_ADMIN);
                }
                ...
                if (!StringUtils.isBlank(entity.getPhone())) {
                    alimTalk.sendAlimTalk(entity.getPhone(), entity.getMailParam(herediumProperties), AlimTalkTemplate.TICKET_REFUND_GROUP);
                }
                alimTalk.cancelAlimTalk(entity.getSmsRequestId());
            }
    ...
    public boolean insertGroup(PostAdminTicketGroupRequest dto) {
        ...
        alimTalk.sendAlimTalk(ticket.getPhone(), ticket.getMailParam(herediumProperties), AlimTalkTemplate.TICKET_ISSUANCE_GROUP);
        List<String> smsRequestId = alimTalk.sendAlimTalk(ticket.getPhone(), ticket.getMailParam(herediumProperties), AlimTalkTemplate.TICKET_INFORMATION, ticket.getStartDate().minusDays(1).withHour(10));
    ...
    public boolean insertInvite(PostAdminTicketInviteRequest dto) {
        ...
        tickets.forEach(ticket -> {
            Map<String, String> mailParam = ticket.getMailParam(herediumProperties);
            mailParam.put("startDate", ticket.getStartDate().format(DateTimeFormatter.ofPattern("yyyy-MM-dd")));
            emailWithParameter.add(new EmailWithParameter(ticket.getEmail(), mailParam));
            NCloudBizAlimTalkMessage message = new NCloudBizAlimTalkMessageBuilder()
                    .variables(ticket.getMailParam(herediumProperties))
                    .to(ticket.getPhone())
     ...
     * 관리자 - 티켓 통계
     */
    public List<GetAdminTicketStatisticsDashboardResponse> statisticsDashboard(Long id, TicketKindType kind) {
        GetAdminTicketStatisticsDashboardResponse total = statisticsRepository.dashboard(kind, id, null);
        total.setType("총 예매");

        List<GetAdminTicketStatisticsDashboardResponse> types = ProjectPriceType.getDefault().stream()
                .map(type -> statisticsRepository.dashboard(kind, id, type.getDesc()))
                .collect(Collectors.toList());

        GetAdminTicketStatisticsDashboardResponse etc = new GetAdminTicketStatisticsDashboardResponse("기타",
                total.getNumber() - types.stream().mapToInt(GetAdminTicketStatisticsDashboardResponse::getNumber).sum(),
                total.getPrice() - types.stream().mapToInt(type -> Math.toIntExact(type.getPrice())).sum()
        );
    ...
    public void refund(Ticket entity) {
        ...
        if (!StringUtils.isBlank(entity.getEmail())) {
            cloudMail.mail(entity.getEmail(), entity.getMailParam(herediumProperties), MailTemplate.TICKET_REFUND_USER);
        }
        if (!StringUtils.isBlank(entity.getPhone())) {
            alimTalk.sendAlimTalk(entity.getPhone(), entity.getMailParam(herediumProperties), AlimTalkTemplate.TICKET_REFUND_USER);
        }
        alimTalk.cancelAlimTalk(entity.getSmsRequestId());
    }
    ...
    public PostAdminTicketQrResponse qrUse(PostTicketQrRequest dto) {
        ...
        if (entity.getState() == TicketStateType.ADMIN_REFUND || entity.getState() == TicketStateType.USER_REFUND) {
        ...
        if (now.isBefore(entity.getStartDate().toLocalDate())) {
            throw new ApiException(ErrorCode.NOT_OPEN, String.format("시작되지 않은 %s입니다.", entity.getKind().getDesc()));
        }
        if (now.isAfter(projectInfo.getEndDate().toLocalDate())) {
            throw new ApiException(ErrorCode.THE_END, String.format("종료된 %s입니다.", entity.getKind().getDesc()));
        }
        boolean isMatch = dto.getAllows().stream().anyMatch(permit -> entity.getKindId().longValue() == permit.getId() && entity.getKind().equals(permit.getKind()));
        if (!isMatch) {
    ...
    public boolean groupMail(PostTicketGroupMailRequest dto) {
        ...
        cloudMail.mail(herediumProperties.getEmail(), dto.toMap(userPrincipal, herediumProperties), MailTemplate.REQUEST_GROUP);
        return true;
    }
    ...
    public boolean coffeeComplete(Long id) {
        ...
        param.put("info", entity.getPrices()
                .stream()
