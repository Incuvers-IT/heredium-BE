...
public class AccountService {
    ...
    public GetUserAccountResponse get(String password) {
        ...
        if (userPrincipal.getProvider() == OAuth2Provider.EMAIL && !bCryptPasswordEncoder.matches(password, userPrincipal.getPassword())) {
    ...
    public PostLoginResponse insert(HttpServletResponse response, PostAccountRequest dto) {
        ...
        PostLoginResponse res = authService.login(response, new PostLoginRequest(dto.getEmail(), dto.getPassword()), false);
    ...
    public boolean update(Long id, PutAdminAccountRequest dto) {
        ...
        if (entity.getProviderType() == OAuth2Provider.EMAIL && !entity.getEmail().equals(dto.getEmail()) && isExistEmail(dto.getEmail())) {
            throw new ApiException(ErrorCode.DUPLICATE_EMAIL);
        }
    ...
    public Page<GetAccountTicketGroupResponse> ticketGroup(GetAccountTicketGroupRequest dto, Pageable pageable) {
        return accountRepository.search(dto, pageable);
    }

    public Page<GetAccountTicketInviteResponse> ticketInvite(GetAccountTicketInviteRequest dto, Pageable pageable) {
        return accountRepository.search(dto, pageable);
    }
    ...
    public Page<GetAdminAccountTicketResponse> ticketByAccount(Long id, Boolean isCoffee, Pageable pageable) {
        ...
        return ticketRepository.findAllByAccount_IdAndKindInOrderByCreatedDateDesc(id, ticketKindTypes, pageable)
    ...
    public List<GetAuthFindIdResponse> findId(String encodeData) {
        ...
        return accounts.stream().map(GetAuthFindIdResponse::new)
                .collect(Collectors.toList());
    }

    public String findPwByPhone(String email, String encodeData) {
        PostNiceIdEncryptResponse info = niceIdService.decrypt(encodeData);
        boolean isExist = accountRepository.existsByEmailAndAccountInfo_PhoneAndProviderType(email, info.getMobileNo(), OAuth2Provider.EMAIL);
        if (!isExist) {
    ...
    public boolean findPwByEmail(String email, String redirectUrl) {
        boolean isExist = accountRepository.existsAccountByEmailAndProviderType(email, OAuth2Provider.EMAIL);
        if (!isExist) {
            throw new ApiException(ErrorCode.USER_NOT_FOUND);
        }
        AuthToken authToken = mailTokenProvider.createAuthToken(email, new Date(new Date().getTime() + appProperties.getAuth().getMailTokenExpiry().toMillis()));
    ...
    public boolean changePw(PostAuthFindPwRequest dto) {
        ...
        Account account = accountRepository.findByEmailEqualsAndProviderType(email, OAuth2Provider.EMAIL);
