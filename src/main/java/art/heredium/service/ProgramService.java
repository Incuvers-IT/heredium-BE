...
public class ProgramService {
    ...
    public List<GetAdminProgramExposeTitleResponse> exposeTitle() {
        return programRepository.findAllByEndDateAfterNow()
                .stream()
    ...
    public GetUserProgramDetailRoundResponse detailRound(Long id) {
        ...
        List<Ticket> tickets = ticketRepository.findAllByRound(bookingStartDate.atTime(LocalTime.MIN), bookingEndDate.atTime(LocalTime.MAX), TicketKindType.PROGRAM, entity.getId());
    ...
    public Object detailRoundInfo(Long id, LocalDate date, String encodeData) {
        ...
        List<Ticket> tickets = ticketRepository.findAllByRound(date.atTime(LocalTime.MIN), date.atTime(LocalTime.MAX), TicketKindType.PROGRAM, entity.getId());
        Map<String, Long> ticketTotalNumber = getTicketTotalNumber(tickets);
        Long ticketNumber = ticketRepository.sumTicketNumber(accountId, nonUserId, date.atStartOfDay(), TicketKindType.PROGRAM, entity.getId());
        List<ProgramRound> rounds = entity.getRounds().stream()
