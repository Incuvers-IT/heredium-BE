...
public class PolicyService {
    ...
    public Page<GetAdminPolicyResponse> list(@Valid GetAdminPolicyRequest dto, Pageable pageable) {
        ...
        return policyRepository.search(dto, pageable)
    ...
    public List<GetUserPolicyResponse> listByUser(PolicyType type) {
        return policyRepository.listByUser(type.getCode()).stream().map(GetUserPolicyResponse::new).collect(Collectors.toList());
    }
    ...
    public boolean postCheck(GetAdminPolicyPostCheckRequest dto) {
        ...
        return (!dto.getPostDate().isAfter(now) && (entity == null || !entity.getPostDate().isAfter(dto.getPostDate())));
    }
}