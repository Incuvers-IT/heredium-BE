...
public class CoffeeService {
    ...
    public List<GetAdminCoffeeExposeTitleResponse> exposeTitle() {
        return coffeeRepository.findAllByEndDateAfterNow()
                .stream()
    ...
    private Map<String, Long> getTicketUsedCount(List<Ticket> tickets) {
        return tickets.stream().filter(ticket -> ticket.getState() == TicketStateType.USED)
                .collect(Collectors.groupingBy(Ticket::getProjectId, Collectors.summingLong(Ticket::getNumber)));
    }
    ...
    private List<Ticket> getTicketsFromCoffeeRound(Coffee entity) {
        LocalDateTime minStartDate = entity.getRounds().stream().map(CoffeeRound::getStartDate).min(LocalDateTime::compareTo).orElse(null);
        LocalDateTime maxStartDate = entity.getRounds().stream().map(CoffeeRound::getStartDate).max(LocalDateTime::compareTo).orElse(null);
        return ticketRepository.findAllByRound(minStartDate, maxStartDate, TicketKindType.COFFEE, entity.getId());
    }
    ...
    public GetUserCoffeeDetailRoundResponse detailRound(Long id) {
        ...
        List<Ticket> tickets = ticketRepository.findAllByRound(bookingStartDate.atTime(LocalTime.MIN), bookingEndDate.atTime(LocalTime.MAX), TicketKindType.COFFEE, entity.getId());
        Map<LocalDateTime, Long> ticketTotalNumber = tickets.stream()
                .collect(Collectors.groupingBy(Ticket::getStartDate, Collectors.summingLong(Ticket::getNumber)));
        List<LocalDate> holidays = holidayRepository.findAllByDayBetween(bookingStartDate, bookingEndDate);
    ...
    public Object detailRoundInfo(Long id, LocalDate date, String encodeData) {
        ...
        if (authentication != null && authentication.isAuthenticated() && !authentication.getPrincipal().equals("anonymousUser")) {
            ...
            NonUser nonUser = nonUserRepository.findByPhoneAndHanaBankUuidIsNull(info.getMobileNo()).orElse(null);
            if (nonUser != null) {
        ...
        List<Ticket> tickets = ticketRepository.findAllByRound(date.atTime(LocalTime.MIN), date.atTime(LocalTime.MAX), TicketKindType.COFFEE, entity.getId());
        Map<String, Long> ticketTotalNumber = getTicketTotalNumber(tickets);
        Long ticketNumber = ticketRepository.sumTicketNumber(accountId, nonUserId, date.atStartOfDay(), TicketKindType.COFFEE, entity.getId());
        List<CoffeeRound> rounds = entity.getRounds().stream()
