...
public class TicketPayService {
    ...
    public PostUserTicketResponse insert(TossPaymentsPayRequest dto) {
        ...
        TicketUserInfo ticketUserInfo = jwtRedisUtil.getData("ticketUserInfo-" + dto.getOrderId(), TicketUserInfo.class);
        ...
        try {
            ...
            alimTalk.sendAlimTalk(entity.getPhone(), entity.getMailParam(herediumProperties), AlimTalkTemplate.TICKET_ISSUANCE);
            List<String> smsRequestId = alimTalk.sendAlimTalk(entity.getPhone(), entity.getMailParam(herediumProperties), AlimTalkTemplate.TICKET_INFORMATION, entity.getStartDate().minusDays(1).withHour(10));
    ...
    private Ticket createTicket(TicketOrderInfo dto, TicketUserInfo ticketUserInfo, String uuid) {
        ...
        List<TicketPrice> ticketPrices = info.getPrices()
                .stream()
                .map(price -> new TicketPrice(null, price.getType(), price.getNumber(), (isDiscount) ? price.getDiscountPrice() : price.getPrice(), price.getPrice(), isDiscount ? price.getNote() : null))
                .collect(Collectors.toList());

        Account account = ticketUserInfo.getAccountId() != null ? accountRepository.findById(ticketUserInfo.getAccountId()).orElse(null) : null;
        NonUser nonuser = ticketUserInfo.getNonUserId() != null ? nonUserRepository.findById(ticketUserInfo.getNonUserId()).orElse(null) : null;
        return new Ticket(ticketPrices, ticketUserInfo, info.getKind(), info.getKindId(), info.getRoundId(), account, nonuser, uuid, info.getTitle(), info.getRoundStartDate(), info.getRoundEndDate());
    }
}