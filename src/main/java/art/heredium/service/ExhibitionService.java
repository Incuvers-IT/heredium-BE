...
public class ExhibitionService {
    ...
    public List<GetAdminExhibitionExposeTitleResponse> exposeTitle() {
        return exhibitionRepository.findAllByEndDateAfterNow()
                .stream()
    ...
    public GetUserExhibitionDetailRoundResponse detailRound(Long id) {
        ...
        List<Ticket> tickets = ticketRepository.findAllByRound(bookingStartDate.atTime(LocalTime.MIN), bookingEndDate.atTime(LocalTime.MAX), TicketKindType.EXHIBITION, entity.getId());
    ...
    public Object detailRoundInfo(Long id, LocalDate date, String encodeData) {
        ...
        List<Ticket> tickets = ticketRepository.findAllByRound(date.atTime(LocalTime.MIN), date.atTime(LocalTime.MAX), TicketKindType.EXHIBITION, entity.getId());
        Map<String, Long> ticketTotalNumber = getTicketTotalNumber(tickets);
        Long ticketNumber = ticketRepository.sumTicketNumber(accountId, nonUserId, date.atStartOfDay(), TicketKindType.EXHIBITION, entity.getId());
        List<ExhibitionRound> rounds = entity.getRounds().stream()
