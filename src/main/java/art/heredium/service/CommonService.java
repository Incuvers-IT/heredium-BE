...
public class CommonService {
  ...
  public List<GetAdminAppProjectResponse> projects() {
    ...
    response.addAll(
        exhibitions.stream().map(GetAdminAppProjectResponse::new).collect(Collectors.toList()));
    response.addAll(
        programs.stream().map(GetAdminAppProjectResponse::new).collect(Collectors.toList()));
    response.addAll(
        coffees.stream().map(GetAdminAppProjectResponse::new).collect(Collectors.toList()));
    return response;
  }
}
