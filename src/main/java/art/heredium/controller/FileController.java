...
public class FileController {
            ...
            if (Constants.valid(file, FilePathType.TEMP)) {
                ...
                if (type != null) {
                    ...
                    if (type.getSmall() != null) {
                        Storage small = cloudStorage.upload(ImageResize.resizeImage(file, type.getSmall()[0], type.getSmall()[1]), FilePathType.TEMP.getPath());
                        resizeImage.setSmall(small.getSavedFileName());
                    }
                    if (type.getMedium() != null) {
                        Storage medium = cloudStorage.upload(ImageResize.resizeImage(file, type.getMedium()[0], type.getMedium()[1]), FilePathType.TEMP.getPath());
                        resizeImage.setMedium(medium.getSavedFileName());
                    }
                    if (type.getLarge() != null) {
                        Storage large = cloudStorage.upload(ImageResize.resizeImage(file, type.getLarge()[0], type.getLarge()[1]), FilePathType.TEMP.getPath());
    ...
    public ResponseEntity download(@RequestParam("path") String path, @RequestParam(value = "fileName", required = false) String fileName) throws IOException {
        byte[] bytes = cloudStorage.getByte(path);
        String downloadName = URLEncoder.encode(fileName != null ? fileName : path, "UTF-8").replaceAll("\\+", "%20");
