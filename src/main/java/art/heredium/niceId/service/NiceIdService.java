...
public class NiceIdService {
  ...
  public String encrypt(PostNiceIdEncryptRequest dto) {
    ...
    //        attr.getRequest().getSession().setAttribute("REQ_SEQ", sRequestNumber);    // 해킹등의 방지를
    // 위하여 세션을 쓴다면, 세션에 요청번호를 넣는다.
    //        System.out.println("encrypt : " +
    // attr.getRequest().getSession().getAttribute("REQ_SEQ"));
    ...
    String sPlainData =
        "7:REQ_SEQ"
            + sRequestNumber.getBytes().length
            + ":"
            + sRequestNumber
            + "8:SITECODE"
            + SITE_CODE.getBytes().length
            + ":"
            + SITE_CODE
            + "9:AUTH_TYPE"
            + sAuthType.getBytes().length
            + ":"
            + sAuthType
            + "7:RTN_URL"
            + sReturnUrl.getBytes().length
            + ":"
            + sReturnUrl
            + "7:ERR_URL"
            + sErrorUrl.getBytes().length
            + ":"
            + sErrorUrl
            + "9:CUSTOMIZE"
            + customize.getBytes().length
            + ":"
            + customize;
  ...
  public PostNiceIdEncryptResponse decrypt(String encodeData) {
    ...
    if (iReturn == 0) {
      ...
      //            ServletRequestAttributes attr = (ServletRequestAttributes)
      // RequestContextHolder.currentRequestAttributes();
      //            String session_sRequestNumber = (String)
      // attr.getRequest().getSession().getAttribute("REQ_SEQ");
      //            System.out.println("response.getRequestNumber() : " +
      // response.getRequestNumber());
      //            System.out.println("decrypt : " + session_sRequestNumber);
      //            if (response.getRequestNumber() == null || session_sRequestNumber == null ||
      // !response.getRequestNumber().equals(session_sRequestNumber)) {
      if (response.getRequestNumber() == null
          || jwtRedisUtil.getData(response.getRequestNumber(), Boolean.class) == null) {
